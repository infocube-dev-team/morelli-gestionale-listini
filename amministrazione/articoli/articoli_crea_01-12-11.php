...
//				$dec = $c1list-$intpart;
	...
	if(isset($_GET['Ricalcola'])){
	   if($_GET['cm31']!=0){
		$diff31 = str_replace(",", ".", $_GET['cm31'])-$c1listf;
	   }
	   if($_GET['cm100']!=0){
		$diff100 = str_replace(",", ".", $_GET['cm100'])-$c2listf;
	   }
	   if($_GET['cm200']!=0){
		$diff200 = str_replace(",", ".", $_GET['cm200'])-$c3listf;
	   }
	   if($_GET['cm300']!=0){
		$diff300 = str_replace(",", ".", $_GET['cm300'])-$c4listf;
	   }
	   if($_GET['cm400']!=0){
		$diff400 = str_replace(",", ".", $_GET['cm400'])-$c5listf;
	   }
	   if($_GET['cm500']!=0){
		$diff500 = str_replace(",", ".", $_GET['cm500'])-$c6listf;
	   }
...
//----------------------------------------Aggiorno l' articolo e il listino per differenze manuali prezzi------------
 //mysql_query("DELETE FROM listino WHERE id_articolo = '$id'");
  ...
  //mysql_query("INSERT INTO listino (id_articolo, 500mt, 400mt, 300mt, 200mt, 100mt, 31mt, 1mt)
   //VALUES ('$id', '$c6listf', '$c5listf', '$c4listf', '$c3listf', '$c2listf', '$c1listf', '$c1listf')");
